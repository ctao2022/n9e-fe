...
import { clusterInfo,Device,mockDevices} from './clusterInfo';
  ...
  const renderRack = (rackId: string) => {
    ...
    return (
      <div key={rackId} style={{ margin: '0 10px', width: '48%' }}>
        <h3 style={{ textAlign: 'center', fontSize: 12, marginBottom: 10 }}>
          机柜 {rackId}
        </h3>
        {/* 渲染设备 */}
  ...
  const renderMessageBox = (type: string) => {
    ...
    React.useEffect(() => {
      if (messageBoxRef.current) {
        ...
        // 对于所有类型，当消息数量大于6时启用滚动
        setShouldScroll(filteredMessages.length > 5);
      }
